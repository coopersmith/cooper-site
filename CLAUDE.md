...
- **`_images/`** - The image library: one metadata record per image under the configured library roots (see **Image library** below), maintained by `image_library.rb`.
...
**Image library** — the site's CMS for images, WordPress-media-library style: one library of originals, one `_images/*.md` record per image, and every surface is a query over the records. Drop an image under a library root (`image_library.roots` in `_config.yml` — currently `assets/photos/`) and `image_library.rb` mints its record, machine-filling EXIF facts *at build time* so served images never need to carry EXIF themselves (date, geocoded location, `width`/`height` orientation-corrected, `camera` normalized to e.g. "Leica Q3", `lens`). Editorial fields are yours to add by editing the record: `surfaces` is the routing — the list of site surfaces the image appears on (`diary` = the `/photos/` visual diary; future galleries are new tags, not new mechanisms — e.g. a best-of is `site.images | where_exp` on a `best` surface, a Leica page a query on `camera`). New records get their root's default surfaces from `_config.yml`. The invariant that makes hand-editing safe: **machine fills, human overrides** — apart from `image` (the path, machine-owned), the generator only writes fields a record doesn't have, so corrections stick. The record's filename-derived slug is the image's permanent ID (drives the `/photos/<slug>/` URL; a rename orphans the old record — its curation must be moved over by hand). Records live in this repo, not the vault, so unlike `_notes/` they are edited here.
...
- **`image_library.rb`** - Maintains the image library (see **Image library** above): ensures every image under the configured roots has an `_images/*.md` record, extracts EXIF (date, dimensions, camera, lens), reverse geocodes GPS via OpenStreetMap (once per image — only when the record has no `location`), deletes records whose image is gone, and syncs new/upgraded records into the in-memory collection so they render the same build. Fill-don't-clobber on every field except `image`
