...
- **`places_geocode.rb`** - Last-resort country lookup for Places, so a brand-new city doesn't need a human before it clusters. Called by `places.rb` only when `_data/places_geo.yml`, the destination notes and the venue's own `loc` chain all come up empty; asks OpenStreetMap Nominatim what country the venue's coordinates are in, one call per unknown *city* (never per venue). Answers are cached in `_places_geo_cache.yml` in the same `City: Country` shape as `places_geo.yml`, so a generated line can be promoted to a curated one by moving it between files — commit the cache and the steady state is zero network calls per build. The cache is deliberately **not** in `_data/`: Jekyll parses that directory itself before any plugin runs and aborts the build on a YAML syntax error, so a half-written cache there would take the whole site down; under a leading underscore Jekyll neither reads nor publishes it, and a corrupt file degrades to one warning plus a re-fetch. Guardrails, because this runs inside a deploy: `MAX_LOOKUPS` (25) per build, a circuit breaker that stops after `MAX_FAILURES` (2) *distinct cities* fail, a per-city failure memo so several venues in one unreachable city don't each burn a lookup, 5s timeouts, 1 req/sec and an identifying User-Agent (both Nominatim policy), and write-then-rename. Every failure is soft — the city just lands in "Elsewhere" as before. `PLACES_GEOCODE=0` disables it; `PLACES_GEOCODE_ENDPOINT` points it at a self-hosted Nominatim or a test stub.
