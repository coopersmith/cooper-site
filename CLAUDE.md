...
The photo section is **one front door with two views**, both full-bleed and linked by a frosted segmented toggle (`_includes/photo-view-toggle.html`) that sits beside the back button on each:

- **`/photos/` — Selects** (`photos_selects` layout): a hand-curated edit, one frame at a time, each with a quiet `place · year` caption. This is the default because the section is a recommendation, the same reasoning that opens `/places/` at a 5+ rating floor rather than at everything.
- **`/photos/all/` — Visual Diary** (`photos_stream` layout): every frame, newest first, with the floating date/location pill. The archive, and the target of a single photo page's "All Photos" link (prev/next there walks the whole date-sorted collection).

The toggle is **navigation, not a client-side filter** — two real pages, so each is shareable and the selects view never pays to render all 69 images. Adding a third view is one entry in `default.html`'s `full_bleed_layouts` list.

**The edit lives in `_data/photos_best.yml`**, not in photo frontmatter: `photo_exif_generator.rb` rewrites that frontmatter and would drop a hand-added `favorite:` field, and a curated edit is an *order*, not a sort — a YAML list carries the sequence for free. Each entry names a `file:` in `assets/photos/` plus optional `title:`/`place:`/`year:` overrides; everything else (date, geocoded location) is read off the generated collection document, so the file only carries what curation actually adds. The `place:` override earns its keep because Nominatim returns the local script (`京都市, 日本`), which is wrong in a caption.

`photos_best.rb` resolves that file into `site.data.photos_selects` — flat strings for the layout, the way `places.rb` distils venues — at `:lowest` priority, after `photo_exif_generator` (`:low`). It formats the caption place by dropping an implied US country, mirroring what `photo-stream.js` hides in the stream's pill. Every failure degrades with a build warning naming the culprit rather than breaking: an entry whose file is missing is skipped, an entry with no `file:` is skipped, and a missing or empty data file leaves `/photos/` rendering its empty state.
...
- **`photos_best.rb`** - Resolves `_data/photos_best.yml` (the curated edit) into `site.data.photos_selects` for the `/photos/` selects view. See Photos above.
...
- `photos_selects.html` / `photos_stream.html` / `photo.html` - The curated `/photos/` edit, the `/photos/all/` Visual Diary, and single photo pages
- `two-column.html` - Year/content grid (used for travels)

### Styling

All CSS lives in `_sass/` and compiles through `styles.scss` — pages don't carry inline `<style>` blocks. `_photos.scss` owns a `frosted-pill` mixin for the glass chrome floating over photos (back button, view toggle, date pill) — include it **last** in a rule, since it emits only nested blocks and Sass warns on declarations that follow one. `_style.scss` holds the global tokens and base styles; page- or feature-specific styles go in their own partial (`_home.scss`, `_media.scss`, `_places.scss`, `_changelog.scss`, `_listening.scss`, `_scrapbook.scss`, `_photos.scss`, `_cv.scss`), imported after the globals in `styles.scss` so overrides don't need `!important`. Supports light/dark mode via CSS custom properties (`--color-*`). Note: `body` caps content width at 720px, so `.wrapper` max-width overrides above that are no-ops.
